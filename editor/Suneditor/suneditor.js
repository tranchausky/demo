...
SUNEDITOR.defaultLang = {
    ...
    dialogBox: {
        linkBox: { title: "Insert Link", url: "URL to link", text: "Text to display" },
        imageBox: { title: "Insert Image", file: "Select from files", url: "Image URL", resize100: "resize 100%", resize75: "resize 75%", resize50: "resize 50%", resize25: "resize 25%", remove: "remove image" },
        videoBox: { title: "Insert Video", url: "Media embed URL, YouTube", width: "Width", height: "Height" },
        submitButton: "Submit",
    },
};
(function () {
    ...
    var core = function (context) {
        var list = (function () {
            var commandMap = { FONT: context.tool.fontFamily, B: context.tool.bold, U: context.tool.underline, I: context.tool.italic, STRIKE: context.tool.strike };
            var fontFamilyMap = {};
            ...
            return { commandMap: commandMap, fontFamilyMap: fontFamilyMap };
        })();
            ...
            return {
                ...
                toggleFrame: function () {
                    if (!context.argument._wysiwygActive) {
                        var ec = { "&amp;": "&", "&nbsp;": "\u00A0", "&quot;": '"', "&lt;": "<", "&gt;": ">" };
                        var source_html = context.element.source.value.replace(/&[a-z]+;/g, function (m) {
        ...
        var event = (function () {
            ...
            return { cancel_resize_image: cancel_resize_image };
        })();
        var user = (function () {
            return {
                save: function () {
        ...
        return { save: user.save, getContent: user.getContent, setContent: user.setContent, appendContent: user.appendContent, disabled: user.disabled, enabled: user.enabled, show: user.show, hide: user.hide };
    };
    var createEditor = function (options) {
        var lang = SUNEDITOR.lang ? SUNEDITOR.lang : SUNEDITOR.defaultLang;
        var toolBar = function () {
            var html = '<div class="sun-editor-id-toolbar-cover"></div>' + '<div class="tool_module">' + '    <ul class="editor_tool">';
            if (options.showFont) {
                html +=
                    "" +
                    '        <li class="compact_editor">' +
                    '            <button type="button" class="btn_editor btn_font" title="' +
                    lang.toolbar.fontFamily +
                    '" data-display="sub">' +
                    '                <span class="txt sun-editor-font-family">' +
                    lang.toolbar.fontFamily +
                    '</span><span class="img_editor ico_more"></span>' +
                    "            </button>" +
                    ...
                    '                        <li><button type="button" class="btn_edit default" data-command="fontName" data-value="inherit" data-txt="' +
                    lang.toolbar.fontFamily +
                    '" style="font-family:inherit;">' +
                    lang.toolbar.fontFamilyDelete +
                    "</button></li>" +
                ...
                if (options.addFont) {
                    ...
                    for (var i = 0; i < options.addFont.length; i++) {
                        ...
                        html +=
                            '        <li><button type="button" class="btn_edit" data-command="fontName" data-value="' + font.value + '" data-txt="' + font.text + '" style="font-family:' + font.value + '">' + font.text + "</button></li>";
                    }
                    html += "        </ul>";
                }
                html += "        </div>" + "            </div>" + "        </li>";
            ...
            if (options.showFormats) {
                html +=
                    "" +
                    '        <li class="compact_editor">' +
                    '            <button type="button" class="btn_editor btn_size" title="' +
            ...
            html += "</ul>" + "</div>" + '<div class="tool_module">' + '   <ul class="editor_tool">';
            if (options.showBold) {
                html +=
                    "" +
                    '       <li class="compact_editor">' +
                    '           <button type="button" class="btn_editor sun-editor-id-bold" title="' +
                    lang.toolbar.bold +
                    ' (Ctrl+B)" data-command="bold"><div class="ico_bold"></div></button>' +
                    "       </li>";
            }
            if (options.showUnderline) {
                html +=
                    "" +
                    '       <li class="compact_editor">' +
                    '           <button type="button" class="btn_editor sun-editor-id-underline" title="' +
                    lang.toolbar.underline +
                    ' (Ctrl+U)" data-command="underline"><div class="ico_underline"></div></button>' +
                    "       </li>";
            }
            if (options.showItalic) {
                html +=
                    "" + "       <li>" + '           <button type="button" class="btn_editor sun-editor-id-italic" title="' + lang.toolbar.italic + ' (Ctrl+I)" data-command="italic"><div class="ico_italic"></div></button>' + "       </li>";
            }
            if (options.showStrike) {
                html +=
                    "" +
                    "       <li>" +
                    '           <button type="button" class="btn_editor sun-editor-id-strike" title="' +
                    lang.toolbar.strike +
                    ' (Ctrl+D)" data-command="strikethrough"><div class="ico_strike"></div></button>' +
                    "       </li>";
            }
            html += "</ul>" + "</div>" + '<div class="tool_module">' + '    <ul class="editor_tool">';
            if (options.showFontColor) {
                html +=
                    ...
                    "</strong>" +
                    '                <button type="button" class="btn_editor" title="' +
                    lang.toolbar.fontColor +
                    '">' +
            ...
            if (options.showHiliteColor) {
                html +=
                    ...
                    "</strong>" +
                    '            <button type="button" class="btn_editor btn_fbgcolor" title="' +
                    lang.toolbar.hiliteColor +
                    '" data-display="sub">' +
            ...
            html += '<div class="tool_module">' + '    <ul class="editor_tool">';
            if (options.showAlign) {
                html +=
                    ...
                    lang.toolbar.align +
                    "</strong>" +
                    '            <button type="button" class="btn_editor btn_align" title="' +
                    lang.toolbar.align +
                    '" data-display="sub">' +
                    '                <span class="img_editor ico_align_l">' +
                    ...
                    '                        <li><button type="button" class="btn_edit btn_align" data-command="justifyleft" title="' +
                    lang.toolbar.alignLeft +
                    '"><span class="img_editor ico_align_l"></span>' +
                    lang.toolbar.left +
                    "</button></li>" +
                    '                        <li><button type="button" class="btn_edit btn_align" data-command="justifycenter" title="' +
                    lang.toolbar.alignCenter +
                    '"><span class="img_editor ico_align_c"></span>' +
                    lang.toolbar.center +
                    "</button></li>" +
                    '                        <li><button type="button" class="btn_edit btn_align" data-command="justifyright" title="' +
                    lang.toolbar.alignRight +
                    '"><span class="img_editor ico_align_r"></span>' +
                    lang.toolbar.right +
                    "</button></li>" +
                    '                        <li><button type="button" class="btn_edit btn_align" data-command="justifyfull" title="' +
                    lang.toolbar.justifyFull +
                    '"><span class="img_editor ico_align_f"></span>' +
                    lang.toolbar.bothSide +
                    "</button></li>" +
                    "                    </ul>" +
                    "                </div>" +
                    "            </div>" +
            ...
            if (options.showList) {
                html +=
                    ...
                    '                <div class="inner_layer inner_layer_type2">' +
                    '                    <ul class="list_editor">' +
                    '                        <li><button type="button" class="btn_edit" data-command="insertOrderedList" data-value="DECIMAL" title="' +
                    lang.toolbar.orderList +
                    '"><span class="img_editor ico_list ico_list_num"></span></button></li>' +
                    '                        <li><button type="button" class="btn_edit" data-command="insertUnorderedList" data-value="DISC" title="' +
                    lang.toolbar.unorderList +
                    '"><span class="img_editor ico_list ico_list_square"></span></button></li>' +
                    "                    </ul>" +
                    "                </div>" +
                    "            </div>" +
                    "        </li>";
            ...
            if (options.showLine) {
                html +=
                    ...
                    lang.toolbar.line +
                    "</strong>" +
                    '            <button type="button" class="btn_editor btn_line" title="' +
                    lang.toolbar.line +
                    '" data-display="sub">' +
            ...
            if (options.showTable) {
                html +=
                    "" +
                    "        <li>" +
                    '            <button class="btn_editor" title="' +
                    lang.toolbar.table +
                    '" data-display="sub" data-command="table">' +
            ...
            if (options.showLink) {
                html +=
                    ...
                    '            <button class="btn_editor" title="' +
                    lang.toolbar.link +
                    '" data-display="modal" data-command="link">' +
            ...
            if (options.showImage) {
                html +=
                    ...
                    '            <button class="btn_editor" title="' +
                    lang.toolbar.image +
                    '" data-display="modal" data-command="image">' +
            ...
            if (options.showVideo) {
                html +=
                    ...
                    '            <button class="btn_editor" title="' +
                    lang.toolbar.video +
                    '" data-display="modal" data-command="video">' +
            ...
            if (options.showCodeView) {
                html +=
                    ...
                    "        <li>" +
                    '            <button class="btn_editor" title="' +
                    lang.toolbar.htmlEditor +
                    '" data-command="sorceFrame">' +
        ...
        var dialogBox = function () {
            var html = "" + '<div class="modal-dialog-background sun-editor-id-dialog-back" style="display: none;"></div>' + '<div class="modal-dialog sun-editor-id-dialog-modal" style="display: none;">';
            html +=
                "" +
                ...
                '</label><input class="form-control sun-editor-id-linktext" type="text">' +
                "                </div>" +
                "            </div>" +
                '            <div class="modal-footer">' +
                '                <button type="submit" class="btn btn-primary sun-editor-id-submit-link"><span>' +
            ...
            html +=
                ...
                '</label><input class="form-control sun-editor-id-image-url" type="text">' +
                "                </div>" +
                "            </div>" +
                '            <div class="modal-footer">' +
                '                <button type="submit" class="btn btn-primary sun-editor-id-submit-image"><span>' +
                lang.dialogBox.submitButton +
                "</span></button>" +
                "            </div>" +
                "        </form>" +
            ...
            html +=
                ...
                '                    <div class="size-text"><label class="size-w">' +
                lang.dialogBox.videoBox.width +
                '</label><label class="size-x"> </label><label class="size-h">' +
                lang.dialogBox.videoBox.height +
                "</label></div>" +
                ...
                "            </div>" +
                '            <div class="modal-footer">' +
                '                <button type="submit" class="btn btn-primary sun-editor-id-submit-video"><span>' +
                lang.dialogBox.submitButton +
                "</span></button>" +
                "            </div>" +
                "        </form>" +
        ...
        var imgBtn = function () {
            return (
                "" +
                '<div class="btn-group">' +
                '   <button type="button" data-command="100" title="' +
                lang.dialogBox.imageBox.resize100 +
                '"><span class="note-fontsize-10">100%</span></button>' +
                '   <button type="button" data-command="75" title="' +
                lang.dialogBox.imageBox.resize75 +
                '"><span class="note-fontsize-10">75%</span></button>' +
                '   <button type="button" data-command="50" title="' +
                lang.dialogBox.imageBox.resize50 +
                '"><span class="note-fontsize-10">50%</span></button>' +
                '   <button type="button" data-command="25" title="' +
                lang.dialogBox.imageBox.resize25 +
                '"><span class="note-fontsize-10">25%</span></button>' +
                "</div>" +
                '<div class="btn-group remove">' +
                '   <button type="button" data-command="remove" title="' +
                lang.dialogBox.imageBox.remove +
                '"><span class="image_remove">X</span></button>' +
                "</div>"
            );
        };
        return { toolBar: toolBar, dialogBox: dialogBox, imgDiv: imgDIv, imgBtn: imgBtn };
    };
    var Constructor = function (element, options) {
        ...
        return {
            constructed: { _top: top_div, _toolBar: tool_bar, _editorArea: editor_div, _resizeBar: resize_bar, _dialog: dialog_div, _loding: loding_box, _resizeImg: resize_img, _resizeImgBtn: resize_img_button, _resizeBack: resize_back },
            options: options,
        };
    };
    var Context = function (element, cons, options) {
        ...
        return {
            ...
            user: { videoX: options.videoX, videoY: options.videoY, imageSize: options.imageSize },
        };
    };
